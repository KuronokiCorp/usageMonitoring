...
1. Ship itermon v1.2.0: MCP server (iterm_mcp.py) into the npm package — README section, files list, maybe `itermon mcp` bin; CEO approves publish
2. Dida verification pass over the MCP server (list/read/send tools, safe-session rules)
3. Issue triage with Lizarazu
